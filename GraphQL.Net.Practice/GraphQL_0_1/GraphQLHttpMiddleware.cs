...
namespace GraphQL_0_1
{
    ...
    /// GraphQL的Http中间件
    /// </summary>
    public class GraphQLHttpMiddleware<TSchema> where TSchema:Schema
    {
        ...
        public GraphQLHttpMiddleware(RequestDelegate next,string path)
        {
            this._next = next;
            this._graphqlPath = path;
        }

        public async Task InvokeAsync(HttpContext context)
        {
            //请求过滤
            if (context.WebSockets.IsWebSocketRequest || !context.Request.Path.StartsWithSegments(_graphqlPath))
            {
                await _next(context);
                return;
            }
